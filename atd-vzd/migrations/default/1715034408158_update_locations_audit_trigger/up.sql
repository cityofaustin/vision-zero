...
BEGIN
    ...
    -- The geometry will be scaled up/down based on a positive or negative value in feet.
    -- The feet will then be converted to meters, which in turn is passed to the
    -- ST_Expand function, and the new geometry is set based on the value it returns.
    IF(NEW.scale_factor IS NOT NULL AND NEW.geometry IS NOT NULL) THEN
        NEW.geometry = ST_Buffer(ST_SetSRID(NEW.geometry,4326), (NEW.scale_factor/3.2808)*0.0000089);
    END IF;
    ...
    ------------------------------------------------------------------------------------------
    -- If we have a geometry, then calculate the centroid lat/longs
...
create trigger atd_txdot_location_audit_log before
insert
    or
update
    on
    public.atd_txdot_locations for each row execute function atd_txdot_locations_updates_audit_log();
