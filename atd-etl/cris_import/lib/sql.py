...
def get_column_operators(
    ...
    for column in target_columns:
        ...
        if column[ "column_name" ] in source:
            ...
            comparison_clause = f"""
                (  (cris_facts.{output_map[table]}.{column['column_name']} = {DB_IMPORT_SCHEMA}.{table}.{column['column_name']})
                OR (cris_facts.{output_map[table]}.{column['column_name']} IS NULL AND {DB_IMPORT_SCHEMA}.{table}.{column['column_name']} IS NULL)
                """
            ...
            if column["data_type"] in ('character varying', 'text'):
                comparison_clause += f"""
                OR (cris_facts.{output_map[table]}.{column['column_name']} IS null and {DB_IMPORT_SCHEMA}.{table}.{column['column_name']} = '')
                OR (cris_facts.{output_map[table]}.{column['column_name']} = '' and {DB_IMPORT_SCHEMA}.{table}.{column['column_name']} IS NULL)  
                """
            comparison_clause += ")"
            ...
            column_aggregator = f"""
                case when not coalesce(
                    cris_facts.{output_map[table]}.{column['column_name']} = {DB_IMPORT_SCHEMA}.{table}.{column['column_name']}
                    or
                    (cris_facts.{output_map[table]}.{column['column_name']} is null and {DB_IMPORT_SCHEMA}.{table}.{column['column_name']} is null)
...
def check_if_update_is_a_non_op(
    ...
    sql += f"from cris_facts.{output_map[table]}\n"
    sql += (
...
def get_key_clauses(table_keys, output_map, table, source, DB_IMPORT_SCHEMA):
    ...
    for key in table_keys[output_map[table]]:
        if not source[key]:
            ...
            return None, None
        public_key_clauses.append(f"cris_facts.{output_map[table]}.{key} = {source[key]}")
...
def fetch_target_record(pg, output_map, table, public_key_sql):
    ...
    sql = f"""
    select * 
    from cris_facts.{output_map[table]}
    where 
    {public_key_sql}
    """
    cursor = pg.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
...
def form_update_statement(
    ...
    sql = "update cris_facts." + output_map[table] + " set "
...
def form_insert_statement(
    ...
    sql = f"insert into cris_facts.{output_map[table]} "
...
def get_linkage_constructions(key_columns, output_map, table, DB_IMPORT_SCHEMA):
    ...
    for column in key_columns:
        linkage_clauses.append(
            f"cris_facts.{output_map[table]}.{column} = {DB_IMPORT_SCHEMA}.{table}.{column}"
        )
...
def get_output_column_types(pg, output_table):
    ...
    # Beware the f-string. But I trust cris_facts.
    sql = f"""
    SELECT
        column_name,
        data_type,
    ...
    for column in changed_columns["changed_columns"]:
        ...
        sql = f"""
            select 
                ...
                cris_facts.{output_map[table]}.{column}::text as cris_{column}
            from cris_facts.{output_map[table]}
