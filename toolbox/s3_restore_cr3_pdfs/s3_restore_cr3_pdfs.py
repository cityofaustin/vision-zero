...
# https://github.com/cityofaustin/atd-airflow/blob/production/dags/python_scripts/atd_vzd_cr3_scan_pdf_records.py#L24
...
try:
    argparse = argparse.ArgumentParser(
        description="Utility to restore last valid PDF in S3 for ATD VZ"
    )
...
try:
    if not args.i_understand:
        ...
        print("")
        print(
            "If there is a 'application/pdf' stored as a previous version of a specified"
        )
        print(
            "crash's CR3, this program will restore that file to the current version."
        )
        print("")
...
if args.production:
    ...
    try:
        ...
        log.error(
            "Production flag is not used and staging doesn't appear in the Hasura endpoint URL"
        )
        sys.exit(1)
...
for crash in crashes:
    ...
    try:
        ...
        cr3_metadata = requests.post(
            HASURA_ENDPOINT,
            headers=HEADERS,
            data=json.dumps({"query": get_metadata, "variables": {"crashId": crash}}),
    ...
    versions = sorted(
        s3_resource.Bucket(bucket).object_versions.filter(Prefix=key),
    ...
    for version in versions:
        ...
        encoding = magic.Magic(flags=magic.MAGIC_MIME_ENCODING).id_buffer(
            previous_version
        )
        ...
        if mime_type != "application/pdf":
            ...
            log.info(
                "Version "
                + obj.get("VersionId")
                + " is acceptable for restoration because it is a "
                + mime_type
            )
        ...
        cr3_metadata["last_update"] = datetime.datetime.now().strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        ...
        s3_resource.Object(bucket, key).copy_from(
            CopySource={"Bucket": bucket, "Key": key, "VersionId": obj.get("VersionId")}
        )
        ...
        affected_rows = requests.post(
            HASURA_ENDPOINT,
            headers=HEADERS,
            data=json.dumps(
                {
                    "query": update_cr3_metadata,
                    "variables": {"crashId": crash, "cr3_file_metadata": cr3_metadata},
                }
            ),
