...
import { Card, CardHeader, CardBody, CardFooter, Table, Button, ButtonDropdown, DropdownToggle, DropdownMenu, Input, DropdownItem } from "reactstrap";
import { recommendationsDataMap } from "./recommendationsDataMap";
import { GET_RECOMMENDATIONS, GET_PARTNERS, GET_STATUS } from "../../queries/recommendations";
...
const Recommendations = ({ crashId }) => {
	...
	const { loading: loadingP, error: errorP, data: dataP, refetch: refetchP } = useQuery(GET_PARTNERS);

	// fetch status table using graphQL query
	const { loading: loadingS, error: errorS, data: dataS, refetch: refetchS } = useQuery(GET_STATUS);
	...
	return (
		<Card>
			...
			<CardBody style={{ padding: "5px 20px 20px 20px" }}>
				<Table>
					<thead>
						<tr style={{ width: "100%", "border-top": "0px", "border-bottom": "1px" }}>
							<th style={{ width: "17%", "border-top": "0px", "border-bottom": "1px" }}>
								Coordination Partner: 
							</th>
							<td style={{ width: "30%", "border-top": "0px", "border-bottom": "1px" }}>
								{displayData(recommendation, fieldConfig.fields.coordination_partner_id)}
								{renderDropDown(dataP.atd__coordination_partners_lkp, setPartnerOpen, partnerDropdownOpen )}
							</td>
							<th style={{ width: "7%", "border-top": "0px", "border-bottom": "1px" }}>
								Status: 
							</th>
							<td style={{ width: "46%", "border-top": "0px", "border-bottom": "1px" }}>
								{displayData(recommendation, fieldConfig.fields.recommendation_status_id)}
								{renderDropDown(dataS.atd__recommendation_status_lkp, setStatusOpen, statusDropdownOpen )}
							</td>
						</tr>
					</thead>
					<tbody>
