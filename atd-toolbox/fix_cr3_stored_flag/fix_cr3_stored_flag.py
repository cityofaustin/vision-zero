...
def main(env):
  ...
  crashes_with_cr3_flag = make_hasura_request(query=CRASHES_QUERY, env=env, variables={})
  
  crashes_to_update = find_crashes_with_wrong_flag(crashes_with_cr3_flag["atd_txdot_crashes"], crashes_with_cr3s)

  print("We need to update the cr3_stored_flag for", len(crashes_to_update), "crashes.")
