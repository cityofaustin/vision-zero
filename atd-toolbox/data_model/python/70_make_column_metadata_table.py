...
def main():
    # translate our data model migration column metadata into the structure
    # we will use in the DB
    columns = load_column_metadata()

    column_inserts = []

    for col in columns:
        is_imported_from_cris =  True if col["source"] == "cris" and col["is_cris_column"] else False
