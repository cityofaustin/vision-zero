...
def test_new_image_upload_get_delete_flow(api_url, editor_user_headers, test_image_jpg, cleanup_person_image):
    ...
    res = requests.put(api_url, files=files, headers=editor_user_headers, data=data)
...
def test_upload_png(api_url, editor_user_headers, test_image_png, cleanup_person_image):
...
def test_upsert_update_source_only(api_url, editor_user_headers, test_image_jpg, cleanup_person_image):
    ...
    res = requests.put(api_url, headers=editor_user_headers, data=data)
    assert res.status_code == 200
    assert res.json()["success"] is True
...
def test_upload_no_file_no_source(api_url, editor_user_headers):
...
def test_upload_new_no_file(api_url, editor_user_headers):
...
def test_upload_new_no_source(api_url, editor_user_headers, test_image_jpg):
...
def test_get_invalid_person_id(api_base_url, editor_user_headers):
