...
Setup:
    ...
    export API_BASE_URL="http://0.0.0.0:8085"  # optional
    export TEST_PERSON_ID="102580"  # optional

Run:
    pytest test_person_image.py -v
"""
...
def test_upload_png(api_url, headers):
    ...
    data = {
        'image_source': 'test_source',
        'image_original_filename': 'test.png'
    }
    res = requests.post(api_url, files=files, headers=headers, data=data)
    assert res.status_code == 201
...
def test_upload_invalid_file(api_url, headers):
    ...
    data = {
        'image_source': 'test_source',
        'image_original_filename': 'fake.jpg'
    }
    res = requests.post(api_url, files=files, headers=headers, data=data)
    assert res.status_code == 400
