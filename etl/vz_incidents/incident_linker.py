...
def main(args):
    ...
    logger.info(f"  Found {len(records):,} unprocessed {record_type} records\n")

    if args.dry_run:
        logger.info(f"Dry run: aborting further processing")
        return
    ...
    for record in records:
        ...
        if record["record_incident_number"] and incident_number_match_table_name:
            ...
            if record_type == "crashes" and record["record_responding_agency"] != "apd":
                ...
                logger.info(f"{len(matches)} incident number matches found")
        ...
        if (
            ...
            logger.info(f"{len(matches)} geo-temporal matches found")
        ...
        if vz_incident_matched_ids:
            ...
            if (
                ...
                logger.info(f"Found {len(member_ids) - 1} neighbor incidents")

            # Create new vz_incident
            logger.info(f"Creating new VZ incident")
    ...
    logger.info(f"\n=== Done ===")
    logger.info(f" Records processed: {counts['records_processed']:,}")
    logger.info(f"  VZ incidents created: {counts['created_by_automation']:,}")
    logger.info(
