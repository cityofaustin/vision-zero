-- Add rpt_street_pfx to socrata_export_crashes_view

drop view "public"."socrata_export_crashes_view";

CREATE
OR REPLACE VIEW "public"."socrata_export_crashes_view" AS WITH unit_aggregates AS (
  SELECT
    crashes_1.id,
  ...
  (
    (
      ...
      LEFT JOIN LATERAL (
        ...
        WHERE
          (crashes.id = crash_injury_metrics_view.id)
        LIMIT
          1
      ) cimv ON (true)
    )
    LEFT JOIN LATERAL (
      ...
      FROM
        unit_aggregates unit_aggregates_1
      WHERE
        (crashes.id = unit_aggregates_1.id)
      LIMIT
        1
...
WHERE
  (
    ...
    AND (
      crashes.crash_timestamp < (now() - '14 days' :: interval)
    )
  )
ORDER BY
  crashes.id;
