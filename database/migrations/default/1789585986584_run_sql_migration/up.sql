ALTER VIEW "public"."crashes_list_view"
  RENAME COLUMN is_manual_geocode TO is_manual_review_qa_complete;

-- Recreate crashes_list_view with a new definition for is_manual_review_qa_complete
...
SELECT
  ...
  -- NEW: manual review QA completion flag.
  -- TRUE when the crash is in the manual_qa geolocation provider state AND
  -- every associated unit has a non-null movement_id AND a veh_trvl_dir_id
  -- in the allowed set (NORTH, EAST, SOUTH, WEST, NOT APPLICABLE, UNKNOWN).
  (
    crashes.geolocation_provider_id = 3
    AND NOT EXISTS (
      SELECT 1
      FROM public.units u
      WHERE u.crash_pk = crashes.id
        AND (
          u.movement_id IS NULL
          OR u.veh_trvl_dir_id IS NULL
          OR u.veh_trvl_dir_id NOT IN (1, 3, 5, 7, 9, 11)
        )
    )
...
FROM
  (
    ...
    LEFT JOIN lookups.injry_sev ON (
      crash_injury_metrics_view.crash_injry_sev_id = injry_sev.id
    )
  )
WHERE
  crashes.is_deleted = false
ORDER BY
  crashes.crash_timestamp DESC;
