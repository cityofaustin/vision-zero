...
ALTER VIEW "public"."crashes_list_view"
  RENAME COLUMN is_manual_review_qa_complete TO is_manual_geocode;


-- Recreate the view definition with the old is_manual_geocode def
CREATE
OR REPLACE VIEW "public"."crashes_list_view" AS WITH geocode_status AS (
  SELECT
    cris.id,
    (
      (
        ...
        AND (
          unified.latitude IS DISTINCT
          FROM
            cris.latitude
        )
      )
      OR (
        (unified.longitude IS NOT NULL)
        AND (
          unified.longitude IS DISTINCT
          FROM
            cris.longitude
        )
      )
      ...
      (
        (
          ...
          LEFT JOIN LATERAL (
            ...
            FROM
              crash_injury_metrics_view crash_injury_metrics_view_1
            WHERE
              (crashes.id = crash_injury_metrics_view_1.id)
            LIMIT
              1
